...
impl Xcb {
    ...
    fn destroy_window(&self, event: &xcb::GenericEvent) -> Event<xcb::Window> {
        ...
        debug!("xcb destroy notification for window {:?}", destroy_notify.window());
        self.connection.flush();
...
impl Backend for Xcb {
    ...
    fn new() -> Result<Xcb> {
        ...
        let root = {
            ...
            info!("screen is {}x{}", screen.width_in_pixels(), screen.height_in_pixels());
            screen.root()
        };
        ...
        let values = [
            (xcb::CW_EVENT_MASK, 
             xcb::EVENT_MASK_SUBSTRUCTURE_NOTIFY | xcb::EVENT_MASK_SUBSTRUCTURE_REDIRECT | xcb::EVENT_MASK_STRUCTURE_NOTIFY |
        ...
        {
            let cookie = xcb::change_window_attributes(&conn, root, &values);
            let _ = cookie.request_check();
        }
        conn.flush();

        Ok(Xcb { connection: conn, root: root })
    }
    ...
    fn resize_window(&self, window: Self::Window, width: u32, height: u32) {
        ...
        let values = [
            (xcb::CONFIG_WINDOW_WIDTH as u16, width), (xcb::CONFIG_WINDOW_HEIGHT as u16, height) 
        ];
    ...
    fn move_window(&self, window: Self::Window, x: u32, y: u32) {
        ...
        let values = [
            (xcb::CONFIG_WINDOW_X as u16, x), (xcb::CONFIG_WINDOW_Y as u16, y) 
        ];
        xcb::configure_window(&self.connection, window, &values);
        self.connection.flush();
    }
